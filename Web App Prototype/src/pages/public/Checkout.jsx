...
export default function Checkout() {
  ...
  const initialItems = [ { id: 1, qty: 1 }, { id: 5, qty: 2 } ]
  ...
  function handleProceed() {
    ...
    const orderItems = items.map(it => {
      const p = findProduct(it.id) || {}
      return { id: it.id, qty: it.qty, name: p.name, price: p.price }
    })
              ...
              {items.map(it => {
                ...
                return (
                  <div key={it.id} className="flex items-center justify-between">
                    ...
                    <div className="font-medium">{formatCurrency((p?.price || 0) * it.qty)}</div>
                  </div>
                )
              })}
            </div>
